...
def read_xygraphp1(folder):
    ...
    train_mask_t = torch.tensor(train_mask_t, dtype=torch.float)
...
class XYGraphP1_no_valid(InMemoryDataset):
    r"""
    Args:
    ...
    def raw_dir(self) -> str:
        return osp.join(self.root, self.name, 'raw_no_valid')

    @property
    def processed_dir(self) -> str:
        return osp.join(self.root, self.name, 'processed_no_valid')
